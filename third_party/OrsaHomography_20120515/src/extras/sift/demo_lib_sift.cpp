	...
	int r, int c, float rpos, float cpos, float rx, float cx,siftPar &par)
{
	...
	if (par.IgnoreGradSign) {
		while (ori > M_PI ) ori = float(ori-M_PI);
		while (ori < 0.0f)  ori = float(ori+M_PI);
	} else {
		while (ori > 2*M_PI) ori = float(ori-2*M_PI);
		while (ori < 0.0f  ) ori = float(ori+2*M_PI);
	}
	...
	float mag, float ori, float rx, float cx,siftPar &par)
{
	...
	float oval = static_cast<float>(OriSize * ori / (par.IgnoreGradSign ? M_PI : 2*M_PI));
...
*/
std::pair<float,float> CheckForMatch(keypoint& key, keypointslist& klist,
                                     keypointslist::size_type& min,siftPar &par)
{
	...
	return std::make_pair(distsq1,distsq2);
}
	...
	matchingslist& matchings,siftPar &par)
{
	...
	for (keypointslist::size_type i=0; i<keys1.size(); i++) {
        ...
        if(sqminratio <= 1.0) {
          if(d.first < sqminratio*d.second)
			matchings.push_back( Match(keys1[i].x,keys1[i].y,
                                       keys2[imatch].x,keys2[imatch].y) );
        } else {
          float t = sqminratio*d.first;
          for(keypointslist::size_type j=0; j<keys2.size(); j++)
            if(DistSquared(keys1[i], keys2[j], t,par) < t)
              matchings.push_back( Match(keys1[i].x,keys1[i].y,
                                         keys2[j].x,keys2[j].y) );
        }
	}
